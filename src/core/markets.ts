...
export class Markets {
   ...
   */
  async getMarkets(filters?: MarketFilters): Promise<IMarkets[]> {
    try {
      const response = await this.httpClient.get<IMarketDataReceived[]>(
        "markets"
      );
      ...
      if (!chainData) {
        throw new MarketError(
          `No market data found for chain ID ${this.chainId}`,
      ...
      if (filters) {
        if (filters.symbol) {
          markets = markets.filter(
            (market) =>
              market.symbol.toLowerCase() === filters.symbol!.toLowerCase()
          );
        }
        if (filters.marketId) {
          markets = markets.filter(
            (market) => market.marketId === filters.marketId
          );
        }
      }
   ...
   */
  async getMarketBySymbol(symbol: string): Promise<IMarkets | null> {
    try {
      const markets = await this.getMarkets({ symbol });
      return markets.length > 0 ? markets[0]! : null;
    } catch (error) {
      throw new MarketError(
        `Failed to fetch market for symbol ${symbol}: ${
          error instanceof Error ? error.message : "Unknown error"
        }`,
        { symbol, chainId: this.chainId }
      );
    }
  }
   ...
   */
  async getMarketById(marketId: string): Promise<IMarkets | null> {
    try {
      const markets = await this.getMarkets({ marketId });
      return markets.length > 0 ? markets[0]! : null;
    } catch (error) {
      throw new MarketError(
        `Failed to fetch market for ID ${marketId}: ${
          error instanceof Error ? error.message : "Unknown error"
        }`,
        { marketId, chainId: this.chainId }
      );
    }
  }
  ...
  async simulateTrade(
    ...
    try {
      const response = await this.httpClient.post<IPostTradeDetails>(
   ...
   */
  async getMarketStats(marketId: string): Promise<Partial<IMarkets>> {
    try {
  ...
  async getAvailableSymbols(): Promise<string[]> {
    try {
      ...
      throw new MarketError(
        `Failed to fetch available symbols: ${
          error instanceof Error ? error.message : "Unknown error"
        }`,
        { chainId: this.chainId }
      );
