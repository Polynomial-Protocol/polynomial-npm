...
class PolynomialSDK {
    ...
    static async create(config) {
        ...
        const apiEndpoint = config.apiEndpoint || config_1.NETWORKS.mainnet.apiEndpoint;
        // Create temporary HTTP client to fetch account ID
        const tempHttpClient = new http_1.HttpClient(apiEndpoint, config.apiKey);
        try {
            ...
            if (!account) {
                throw new errors_1.AccountError(`No account found for wallet ${config.walletAddress} on chain ${chainId}. Please ensure the wallet has an active account on the Polynomial platform.`, {
                    walletAddress: config.walletAddress,
                    chainId,
                });
            }
    ...
    async createLimitOrder(marketId, size, acceptablePrice, options) {
        try {
            ...
            throw new errors_1.ValidationError(`Failed to create limit order: ${error instanceof Error ? error.message : "Unknown error"}`, {
                marketId,
                size: size.toString(),
    ...
    async createLimitLongOrder(marketId, size, acceptablePrice, reduceOnly = false) {
        try {
            ...
            throw new errors_1.ValidationError(`Failed to create limit long order: ${error instanceof Error ? error.message : "Unknown error"}`, {
                marketId,
                size: size.toString(),
                acceptablePrice: acceptablePrice.toString(),
                walletAddress: this.walletAddress,
    ...
    async createLimitShortOrder(marketId, size, acceptablePrice, reduceOnly = false) {
        try {
            ...
            throw new errors_1.ValidationError(`Failed to create limit short order: ${error instanceof Error ? error.message : "Unknown error"}`, {
                marketId,
                size: size.toString(),
                acceptablePrice: acceptablePrice.toString(),
                walletAddress: this.walletAddress,
