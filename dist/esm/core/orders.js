...
import { isValidPrivateKey, generateNonce, getWeekFromNowTimestamp, calculateAcceptablePrice, parseUnits, } from "../utils";
/**
...
export class Orders {
    ...
    async getMarketPrice(marketId) {
        try {
            ...
            if (!chainData) {
                throw new ValidationError(`Chain data not found for chain ID ${this.networkConfig.chainId}`, {
    ...
    async createMarketOrder(params, defaultSlippage = 10n) {
        ...
        try {
            ...
            throw new OrderError(`Failed to create market order: ${error instanceof Error ? error.message : "Unknown error"}`, {
                marketId,
                accountId,
                size: size.toString(),
                isLong,
    ...
    async signLimitOrder(sessionKey, order) {
        ...
        try {
            ...
            throw new SigningError(`Failed to sign limit order: ${error instanceof Error ? error.message : "Unknown error"}`, { marketId: order.marketId, accountId: order.accountId });
        }
    }
    /**
    ...
    async submitLimitOrder(marketId, data) {
        try {
            ...
            throw new OrderError(`Failed to submit limit order: ${error instanceof Error ? error.message : "Unknown error"}`, { marketId, orderData: { ...data, id: "REDACTED" } });
        }
    }
    /**
    ...
    async createLimitOrder(params) {
        ...
        try {
            ...
            throw new OrderError(`Failed to create limit order: ${error instanceof Error ? error.message : "Unknown error"}`, {
                marketId,
                accountId,
                size: size.toString(),
                isLong,
